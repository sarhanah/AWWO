...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=085719448074:+62 857-1944-8074\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '.'
blocked = []
limitawal = '20'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '085719448074@s.whatsapp.net'
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/yta?url=${args[0]}&apiKey=${A4wJwkoSak82hj82NWo92}`, {method: 'get'})
				...
				case 'ytmp4':
					if (args.length < 1) return reply('Urlnya mana um?')
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://mhankbarbar.tech/api/ytv?url=${args[0]}&apiKey=${A4wJwkoSak82hj82NWo92}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				/*case 'semoji':
					...
					anu = await fetchJson(`https://mhankbarbars.tech/api/emoji2png?emoji=${teks}&apikey=${A4wJwkoSak82hj82NWo92}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'tulis':
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbar.tech/nulis?text=${teks}&apiKey=${A4wJwkoSak82hj82NWo92}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'clone':
					...
					break*/
                                ...
                                case 'jadwaltV':
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        anu = await fetchJson(`https://mhankbarbar.tech/api/jdtv?ch=${mb}&apiKey=${A4wJwkoSak82hj82NWo92}`, {method: 'get'})
                                        teks = `◪ *JADWAL TV* \n  │\n  └─ ❏ Channel : ${mb} \n◪ *JADWAL* \n${anu.result} `
                                        client.sendMessage(from, teks, text)
                                        await limitAdd(sender)
                                        break
